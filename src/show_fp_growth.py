...
def handleFrequentItemFP():
  result_fp = fpgrowth(data, min_support=0.01, use_colnames=True)
  # result_ap.head(number_get).sort_values('support', ascending=False)
  result_fp = result_fp.sort_values('support', ascending=False)
  return result_fp
...
def get_rules_fp():
  ...
  rules_fp = association_rules(result_fp, metric='lift', min_threshold=1.0)
  return rules_fp
...
def show_fp_growth():
  ...
  if (st.button("Recommend")):
    result_fp = handleFrequentItemFP()
    recommend(freq_itemset=result_fp, ante=item)
